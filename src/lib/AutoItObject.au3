...
If Not IsDeclared("S_OK") Then Global Const $S_OK_ = 0x00000000
If Not IsDeclared("E_NOTIMPL") Then Global Const $E_NOTIMPL_ = 0x80004001
If Not IsDeclared("E_NOINTERFACE") Then Global Const $E_NOINTERFACE_ = 0x80004002
If Not IsDeclared("E_POINTER") Then Global Const $E_POINTER_ = 0x80004003
If Not IsDeclared("E_ABORT") Then Global Const $E_ABORT_ = 0x80004004
If Not IsDeclared("E_FAIL") Then Global Const $E_FAIL_ = 0x80004005
If Not IsDeclared("E_ACCESSDENIED") Then Global Const $E_ACCESSDENIED_ = 0x80070005
If Not IsDeclared("E_HANDLE") Then Global Const $E_HANDLE_ = 0x80070006
If Not IsDeclared("E_OUTOFMEMORY") Then Global Const $E_OUTOFMEMORY_ = 0x8007000E
If Not IsDeclared("E_INVALIDARG") Then Global Const $E_INVALIDARG_ = 0x80070057
If Not IsDeclared("E_UNEXPECTED") Then Global Const $E_UNEXPECTED_ = 0x8000FFFF
...
Func _AutoItObject_ArrayToString($__sName, $__aArray)
	$__sRes = $__sName & " {" ;
	For $i = 0 To UBound($__aArray) - 1
		$__sRes &= $__aArray[$i] & ", " ;
	Next
	Return $__sRes & "}" ;
...
Func QueryInterface($pSelf, $pRIID, $pObj)
	If $pObj = 0 Then Return $E_POINTER_
	Local $sGUID = DllCall("ole32.dll", "int", "StringFromGUID2", "PTR", $pRIID, "wstr", "", "int", 40)[2]
	If (Not ($sGUID = $IID_IDispatch)) And (Not ($sGUID = $IID_IUnknown)) Then Return $E_NOINTERFACE_
...
Func GetIDsOfNames($pSelf, $riid, $rgszNames, $cNames, $lcid, $rgDispId)
	...
	EndSwitch
	If DllStructGetData($tIds, 1) <> -1 Then Return $S_OK_
...
Func GetTypeInfo($pSelf, $iTInfo, $lcid, $ppTInfo)
	...
	If $ppTInfo = 0 Then Return $E_INVALIDARG_
	Return $S_OK_
EndFunc   ;==>GetTypeInfo
...
Func Invoke($pSelf, $dispIdMember, $riid, $lcid, $wFlags, $pDispParams, $pVarResult, $pExcepInfo, $puArgErr)
	...
	If $dispIdMember < -1 Then
		...
		If $dispIdMember = -13 Then ;__lookupSetter
			...
			If Not GetIDsOfNames($pSelf, $riid, $t.str_ptr_ptr, 1, $lcid, DllStructGetPtr($t, "id")) == $S_OK_ Then Return $DISP_E_EXCEPTION
		...
		If $dispIdMember = -18 Then ;__get
			...
			$t.str_ptr_ptr = DllStructGetPtr($t, "str_ptr")
			If Not GetIDsOfNames($pSelf, 0, $t.str_ptr_ptr, 1, $lcid, DllStructGetPtr($t, "id")) = $S_OK_ Then Return $DISP_E_EXCEPTION
		...
		If ($dispIdMember = -8) Then ;__keys
			...
			While 1
				...
				$pProperty = $tProperty.Next
			WEnd
			If $pProperty = 0 Then Return $S_OK_
	...
	If (Not (BitAND($wFlags, $DISPATCH_PROPERTYGET) = 0)) Then
		...
		If Not ($tProperty.__getter = 0) Then
			...
			$oIDispatch = 0
			Return ($iError <> 0) ? $DISP_E_EXCEPTION : $S_OK_
			Return $S_OK_
		EndIf
...
Func VariantInit($tVARIANT)
	...
	If $aRet[0] <> $S_OK_ Then SetError($aRet, 0, $tVARIANT)
	Return 1
EndFunc   ;==>VariantInit
...
Func VariantCopy($tVARIANT_Dest, $tVARIANT_Src)
	...
	If @error <> 0 Then Return SetError(-1, @error, 0)
	If $aRet[0] <> $S_OK_ Then SetError($aRet, 0, 0)
	Return 1
EndFunc   ;==>VariantCopy
...
Func VariantChangeType()
	;TODO
EndFunc   ;==>VariantChangeType

Func VariantChangeTypeEx()
	;TODO
EndFunc   ;==>VariantChangeTypeEx

Func PrivateProperty()
	Return SetError(1, 1, 0)
EndFunc   ;==>PrivateProperty
...
EndFunc   ;==>__AOI_PropertyGetFromName
...
Func __AOI_PropertyGetFromId($pProperty, $iID)
	...
	Next
	Return $tProperty
EndFunc   ;==>__AOI_PropertyGetFromId
